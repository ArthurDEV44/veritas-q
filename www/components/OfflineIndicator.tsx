...
export default function OfflineIndicator({
    ...
    return (
      <>
        {(isOffline || pendingCount > 0) && (
          <div
            className={`animate-[scaleIn_0.3s_ease-out] flex items-center gap-1.5 px-2 py-1 rounded-full text-xs ${
              isOffline
  ...
  if (banner) {
    return (
      <>
        {isOffline && (
          <div className="animate-[slideDown_0.3s_ease-out] fixed top-0 left-0 right-0 z-50 px-4 py-2 bg-amber-500/90 backdrop-blur-sm text-black text-center text-sm font-medium">
  ...
  return (
    <>
      {(isOffline || pendingCount > 0) && (
        <div
          className={`animate-[slideDown_0.3s_ease-out] rounded-xl border p-4 ${
            isOffline
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {!isOffline && !isSyncing && pendingCount > 0 && (
                <button
                  onClick={syncAll}
                  className="transition-transform active:scale-95 mt-3 flex items-center gap-2 px-3 py-1.5 bg-quantum/20 hover:bg-quantum/30 text-quantum rounded-lg text-sm font-medium transition-colors"
                >
...
export function SyncSuccessToast({ onClose }: { onClose: () => void }) {
  return (
    <div
      className="animate-[scaleIn_0.3s_ease-out] fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-3 bg-green-500/90 backdrop-blur-sm text-white rounded-xl shadow-lg"
      onAnimationEnd={() => {
